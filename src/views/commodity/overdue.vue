				...
				<el-table-column prop="slider_image" label="轮播图" min-width="320">
					<template slot-scope="scope">
						...
						<div v-else style="width: 1200px">
							<el-image v-for="(item,index) in JSON.parse(scope.row.slider_image)" :key="index" ref="lazyImgs" class="vx-lazyLoad" :src="item" fit="fit" :preview-src-list="[item]" style="width: 150px; height: 100px">
								<div slot="placeholder" class="image-slot">
									<i class="el-icon-loading"></i>加载中
								</div>
								<div slot="error" class="image-slot">
...
export default {
	...
	methods: {
		...
		getList(num) {
			...
			GetExpirationTimeProduct(this.form)
				.then((res) => {
